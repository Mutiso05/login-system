...
if ($_SERVER["REQUEST_METHOD"] == "POST") { // Checks if the request method is POST
    // Sanitize input data
    $username = trim($_POST['username']); // Retrieves and trims whitespace from the username
    $password = trim($_POST['password']); // Retrieves and trims whitespace from the password

    if (empty($username) || empty($password)) { // Checks if the username or password is empty
        header("Location: error.html"); // Redirects to the error.html page if either is empty
    ...
    // Change this table name if necessary
    ...
    if ($stmt) { // Checks if the statement was successfully prepared
        $stmt->bind_param("ss", $username); // Binds the username parameter to the SQL query
